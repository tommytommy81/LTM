function stat12 = fun_TF_stat(TFR1, TFR2, options)
...
% check on .freq
if length(TFR1.freq) == length(TFR2.freq)
    TFR1.freq = TFR2.freq;
else
    error('frequencies are not consistent for testing\n')
end
...
cfg.method = 'montecarlo';       % use the Monte Carlo Method to calculate the significance probability
        ...
        cfg.numrandomization = options.numrandomization  ;      % number of draws from the permutation distribution
        cfg.ivar  = 1;                   % number or list with indices indicating the independent variable(s)
        
        cfg.channel       = TFR1.label;     % cell-array with selected channel labels
        cfg.latency       = options.test_latency;       % time interval over which the experimental
        % conditions must
        for ch = 1
